...
export default function ResetPasswordClient({ tokenFromParams }) {
  const router = useRouter();
  const searchParams = useSearchParams();

  const token = useMemo(() => {
    return searchParams?.get('token') || tokenFromParams || null;
      ...
      {tokenError ? (
        ...
        <>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <button type="submit" className="w-full pill-btn gradient-btn disabled:opacity-50" disabled={isSubmitting}>
