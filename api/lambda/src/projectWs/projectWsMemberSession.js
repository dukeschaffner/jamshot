...
function resolveGatewayContextFromConnections(connections) {
  ...
  // Connections joined before gateway columns existed / WS restart — still cut
  // sockets in local dev via the control HTTP endpoint.
  if (process.env.NODE_ENV === 'dev' || process.env.PROJECT_WS_LOCAL_CONTROL_URL) {
    return { mode: 'local' };
  }
